 ...
 * @author khactoan9x
 */
public class PlayerTableViewFancy extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("var fnameCell = document.getElementById(++id);");
            out.println("var lnamCell = document.getElementById(++id);");
            ...
            out.printf("var nameInput = createTextInput(fnameCell.innerText, \"%s\");", PlayerLogic.FIRST_NAME);
            out.printf("var nameInput = createTextInput(lnameCell.innerText, \"%s\");", PlayerLogic.LAST_NAME);
            out.printf("var nameInput = createTextInput(dateCell.innerText, \"%s\");", PlayerLogic.JOINED);
            out.printf("var nameInput = createTextInput(emailCell.innerText, \"%s\");", PlayerLogic.EMAIL);
            
            out.println("idCell.innerText = null;");
            out.println("fnameCell.innerText = null;");
            out.println("lnameCell.innerText = null;");
            out.println("dateCell.innerText = null;");
            out.println("emailCell.innerText = null;");
            
            out.println("idCell.appendChild(idInput);");
            out.println("fnameCell.appendChild(nameInput);");
            out.println("lnameCell.appendChild(nameInput);");
            out.println("dateCell.appendChild(nameInput);");
            out.println("emailCell.appendChild(nameInput);");
            out.println("};");
            out.println("}");
            out.println("};");
            out.println("</script>");
            ...
            List<Player> entities = logic.getAll();
            long counter = 0;
            for (Player entity : entities) {
                ...
                out.printf("<td class=\"id\" id=\"%d\" >%d</td>", counter++, entity.getId());
                out.printf("<td class=\"firstName\" id=\"%d\" >%s</td>", counter++, entity.getFirstName());
                out.printf("<td class=\"lastName\" id=\"%d\" >%s</td>", counter++, entity.getLastName());
                out.printf("<td class=\"joined\" id=\"%d\" >%s</td>", counter++, entity.getJoined());
                out.printf("<td class=\"email\" id=\"%d\" >%s</td>", counter++, entity.getEmail());
